...
function Navbar() {
  ...
  const { contentList, setContentList } = useContent();
  const [contentLink, setContentLink] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(contentList.length);
    setContentList([...contentList, contentLink]);
    setContentLink("");
  };
        ...
        <div className="flex items-center">
          <form onSubmit={handleSubmit} className="flex w-full h-full">
            ...
            <input
              ...
              placeholder="Import Content via link"
              value={contentLink}
              onChange={(e) => {
            ...
            />
            <button type="submit"
              className="rounded-r-full bg-slate-100 dark:bg-slate-800 h-10 w-18 flex items-center justify-center border border-slate-200 dark:border-slate-800 transition duration-300 ease hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer border-l-0">
